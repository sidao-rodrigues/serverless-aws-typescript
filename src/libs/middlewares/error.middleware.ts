...
const generateSchemaError = (error: HttpError): SchemaError => {
  return (error.cause as HttpError)?.data
    .reduce((acc: SchemaError, item: ErrorData, _: number, arr: ErrorData): SchemaError => {
      //se arr === 1 então não há body. Depois percorre o array com os erros e insere no arr de stacks
      return {
        ...acc,
        message: arr.length === 1 && (item.data === null || JSON.stringify(item.data) === '{}')
          ? 'Corpo da requisição é obrigatório' 
          : 'Um ou mais campos estão inválidos',
        stack: [
          ...(acc.stack || []),
          {
            schema: item.keyword === 'required' 
...
const schemaErrorMiddleware = (options?: IOptions): middy.MiddlewareObj<APIGatewayProxyEvent, APIGatewayProxyResult> => {
...
const appErrorMiddleware = (options?: IOptions): middy.MiddlewareObj<APIGatewayProxyEvent, APIGatewayProxyResult> => {
  const onError: middy.MiddlewareFn<APIGatewayProxyEvent, APIGatewayProxyResult> = async (request): Promise<void> => {
